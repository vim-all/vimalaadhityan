...
export function CustomCursor() {
    ...
    const handleMouseOver = (e: MouseEvent) => {
      ...
      let interactiveEl = target.closest("a, button, [contenteditable]") as HTMLElement;
        ...
        if (interactiveEl) {
          ...
          if (cursorLabel) {
            setHoverText(cursorLabel.toUpperCase());
          } else if (innerText) {
            setHoverText(innerText.substring(0, 20).toUpperCase());
          } else {
            setHoverText("LOCKED");
          }
        } else {
  ...
  return (
    <>
      <div className="hidden md:block pointer-events-none z-10000 fixed inset-0 overflow-hidden">
        
        {/* Tail 2 (Smallest, furthest back) */}
        <motion.div
            className="absolute w-1.5 h-1.5 bg-accent rounded-full mix-blend-difference"
        ...
        {/* Tail 1 (Medium) */}
        <motion.div
            className="absolute w-2 h-2 bg-accent rounded-full mix-blend-difference"
        ...
        {/* Lead Dot (Heavy) */}
        <motion.div
            className="absolute w-3 h-3 bg-accent rounded-full mix-blend-difference"
        ...
        <motion.div
            className="absolute w-1 h-1 bg-accent rounded-full mix-blend-difference"
        ...
        <motion.div
            className="absolute flex items-center justify-center font-mono text-[10px] tracking-[0.2em] font-black uppercase text-accent"
            style={{ left: leadX, top: leadY, x: "-50%", y: "-50%" }}
        ...
        >
            ...
            <motion.span
              className="absolute -top-10 whitespace-nowrap bg-accent text-background px-3 py-1.5 rounded-[4px] shadow-2xl"
