...
const projects = [
  { id: 1, name: "computehive", color: "#FF4D00", desc: "Decentralized CPU/GPU sharing platform bridging bare-metal resources across a distributed network.", stack: "Go, Tauri, Rust, R2 Storage, Redis, AWS" },
  { id: 2, name: "Vessel", color: "#EC4899", desc: "A high-performance containerization engine engineered from the ground up, achieving radical efficiency in process isolation and resource management.", stack: "Rust" },
  { id: 3, name: "shadow-Code", color: "#3B82F6", desc: "Legacy modernization platform utilizing risk analysis to safely refactor historical codebases.", stack: "Python, AST, TypeScript, LLM" },
  { id: 4, name: "Cryptic Hunt", color: "#E0FF22", desc: "Distributed admin dashboard sustaining 500+ concurrent traffic spikes via aggressive scale-out topologies.", stack: "Next.js, Prisma, CockroachDB" },
  { id: 5, name: "Trading AI", color: "#C055ED", desc: "Chrome extension backed MCP server maintaining WebSocket sessions for Pine Script generation.", stack: "Python, WebSockets, DynamoDB" },
];
...
function ParticleMorphingScene({ themedProjects }: { themedProjects: any[] }) {
  ...
  const shapes = useMemo(() => [
    ...
    generateTorus(PARTICLE_COUNT, 4.0, 0.5),
  ...
  useFrame((state, delta) => {
    ...
    const rawIndex = (offset * shapes.length) - 0.5;
      ...
      <div className="sticky top-0 w-full h-screen bg-background overflow-hidden selection:bg-accent/30">
        ...
        <div className="absolute inset-0 z-20 pointer-events-none">
          {themedProjects.map((p, idx) => (
            <ProjectItem key={p.id} p={p} idx={idx} smoothProgress={smoothProgress} step={1 / (themedProjects.length + 1)} />
          ))}
          ...
          <motion.div
            ...
            style={{
              opacity: useTransform(smoothProgress, [0.84, 0.94], [0, 1]),
              scale: useTransform(smoothProgress, [0.84, 0.94], [0.8, 1]),
